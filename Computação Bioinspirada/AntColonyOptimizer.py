...
class AntColonyOptimizer:
    def __init__(self, ants, evaporation_rate, intensification, alpha=1.0, choose_best=.1):
        """
    ...
    def _initialize(self):
        ...
        self.probability_matrix = (self.pheromone_matrix ** self.heuristic_alpha) * (
                self.heuristic_matrix )  # element by element multiplcation
    ...
    def fit(self, map_matrix, iterations=100, mode='min', early_stopping_count=20, verbose=False):
        ...
        if mode == 'min':
            ...
            return self.best, best_path_coords
        elif mode == 'max':
...
if __name__ == '__main__':
    ...
    for i in range(30):
        optimizer = AntColonyOptimizer(ants=16, evaporation_rate=.3, intensification=2, alpha=1, choose_best=.2)

        best, best_path = optimizer.fit(adj_mat, 100)
        f.write(str(best_path[0])+' ' +str(int(best))+'\n')
    f.close()